...
def test_custom_sampler_n_iters_fallback():
    class CustomLegacySampler(Sampler):
        @property
        def batch_size(self):
            return 10
    ...
    sampler = CustomLegacySampler()
    with pytest.warns(FutureWarning, match="n_iters is deprecated, use/implement n_batches instead"):
        assert sampler.n_batches(100) == 42


def test_custom_sampler_not_implemented():
    ...
    sampler = IncompleteSampler()
    with pytest.raises(NotImplementedError, match="Subclasses of Sampler must implement n_batches"):
        sampler.n_batches(100)
