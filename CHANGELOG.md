...
- Added {class}`annbatch.samplers.CategoricalSampler`: a replacement sampler that takes a {class}`pandas.Categorical` (e.g. `adata.obs["cell_type"].astype("category")`) and guarantees every yielded chunk lies entirely within a single category. Sampling is with replacement; categories are drawn by `category_weights` (uniform by default).
